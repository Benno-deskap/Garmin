...
Sleep analysis — best and worst night, deep sleep trend, REM and SpO2 patterns
Heart rate and HRV trends
Training Readiness and training load balance — including ACWR, aerobic low/high breakdown, and feedback phrase explanation
Fitness age and VO2max estimate — with improvement potential and component breakdown (intensive days, minutes, RHR, BMI)
Race predictions — 5K, 10K, half marathon, marathon
Planned Garmin workouts — name, duration, steps (warmup → interval → cooldown), and advice to execute, adjust or skip based on readiness and ACWR
Heart rate zone distribution
Movement and step analysis — sedentary time and intensity minutes vs WHO goal
Body Battery and stress trends
Weight trend — current weight, 30-day delta, distance to goal, BMI (if data available)
Top correlations between metrics — with exact dates and values
Top 5 sleep recommendations — focused on quality when sleep duration target is met
Top 5 movement recommendations — focused on aerobic high-intensity when step goal is met
Goals for next week — based on largest deficits in intensive training, weight, and sleep quality
